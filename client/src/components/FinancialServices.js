import React, { useEffect, useState } from "react";
import { FaArrowRight, FaStar, FaUsers } from "react-icons/fa";
import ServiceForm from "./ServiceForm"; // Import the ServiceForm
import "./FinancialServices.css";

const LoanServices = ({ onApplyClick }) => {
  const [loanServices, setLoanServices] = useState([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState(null);
  const [hoveredCard, setHoveredCard] = useState(null);
  const [showServiceForm, setShowServiceForm] = useState(false);
  const [selectedService, setSelectedService] = useState(null);

  useEffect(() => {
    const fetchLoanServices = async () => {
      try {
        const response = await fetch("https://api.woodxo.com/api/loan-services");
        if (!response.ok) throw new Error("Failed to fetch loan services");
        const data = await response.json();
        setLoanServices(data);
      } catch (err) {
        console.error("Error:", err);
        setError(err.message);
      } finally {
        setLoading(false);
      }
    };
    fetchLoanServices();
  }, []);

  const handleGetService = (service) => {
    setSelectedService(service);
    setShowServiceForm(true);
  };

  const handleCloseServiceForm = () => {
    setShowServiceForm(false);
    setSelectedService(null);
  };

  if (loading) return <p>Loading loan services...</p>;
  if (error) return <p style={{ color: "red" }}>Error: {error}</p>;

  return (
    <>
      <section className="financial-services-modern">
        <div className="container">
          <div className="section-header-modern">
            <div className="section-badge">Our Services ✨</div>
            <h2>
              Professional&nbsp;<span>Loan Services</span>
            </h2>
            <p>
              Choose from our wide range of financial services tailored to your
              business and personal needs
            </p>
          </div>

          <div className="services-grid-modern">
            {loanServices.map((service, index) => (
              <div
                key={service.id}
                className={`service-card-modern ${
                  hoveredCard === service.id ? "hovered" : ""
                }`}
                onMouseEnter={() => setHoveredCard(service.id)}
                onMouseLeave={() => setHoveredCard(null)}
                style={{ animationDelay: `${index * 0.1}s` }}
              >
                <div className="card-content-modern">
                  <div className="service-image-container">
                    <img
                      src={`https://loancopy.onrender.com/${service.image}`}
                      alt={service.name}
                      className="service-image"
                      onError={(e) =>
                        (e.target.src =
                          "https://via.placeholder.com/100/cccccc?text=No+Image")
                      }
                    />
                  </div>

                  <div className="service-info">
                    <h3>{service.name}</h3>
                    <p className="service-description">{service.description}</p>
                  </div>

                </div>

                <div className="card-actions-modern">
                  <button
                    className="btn-primary-modern"
                    onClick={() => handleGetService(service)}
                  >
                    <span>Get Service</span>
                    <FaArrowRight className="arrow-icon" />
                  </button>
                </div>
              </div>
            ))}
          </div>
        </div>
      </section>

      {/* Service Form Popup */}
      {showServiceForm && (
        <div className="service-form-popup-overlay">
          <div className="service-form-popup-container">
            <button
              className="service-popup-close-btn"
              onClick={handleCloseServiceForm}
            >
              &times;
            </button>
            <div className="service-form-popup-content">
              <ServiceForm 
                onClose={handleCloseServiceForm}
                selectedService={selectedService}
              />
            </div>
          </div>
        </div>
      )}
    </>
  );
};

export default LoanServices;

